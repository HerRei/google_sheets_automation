...
entry_types = ["Record Type A", "Record Type B", "Record Type A", "Record Type B", "Record Type C"]
...
def save_data(data):
    try:
        s = get_service()
        entry_type_name = data['entry_type']
        
        # find the next empty row
        result = s.spreadsheets().values().get(spreadsheetId=SHEET_ID, range=entry_type_name + "!A:A").execute()
        ...
        # append it
        range_name = entry_type_name + "!A" + str(next_row)
        body = {'values': [new_row]}
...
def entry_type():
    ...
    for s in entry_types:
        buttons = buttons + '<button class="btn" name="entry_type" value="' + s + '">' + s + '</button>'
...
def details():
    ...
    <h2>Details info</h2>
    <form method='post'>
    <input type='text' name='reference' placeholder='Details Number'><br><br>
    <button class='btn' name='option' value='1'>Normal</button>
    <button class='btn' name='option' value='2'>Express</button>
    </form>
    """
...
def dest():
    ...
    buttons = ""
    for d in targets:
        buttons = buttons + '<button class="btn" name="target" value="' + d + '">' + d + '</button>'
